...
def extract_detailed_summary(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
    return ai_service.generate_detailed_summary(db, meeting)

@router.post("/{meeting_id}/key-points", response_model=MeetingResponse)
def extract_key_points(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
    return ai_service.generate_key_points(db, meeting)

@router.post("/{meeting_id}/risks", response_model=MeetingResponse)
def extract_risks(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
    return ai_service.generate_risks(db, meeting)

@router.post("/{meeting_id}/open-questions", response_model=MeetingResponse)
def extract_open_questions(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
...
def extract_follow_up_email(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
    return ai_service.generate_follow_up_email(db, meeting)

@router.post("/{meeting_id}/next-agenda", response_model=MeetingResponse)
def extract_next_agenda(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
    return ai_service.generate_next_agenda(db, meeting)

@router.post("/{meeting_id}/ai-title", response_model=MeetingResponse)
def extract_meeting_title(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
    return ai_service.generate_meeting_title(db, meeting)

@router.post("/{meeting_id}/tags", response_model=MeetingResponse)
def extract_tags(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
    return ai_service.generate_tags_and_category(db, meeting)

@router.post("/{meeting_id}/sentiment", response_model=MeetingResponse)
def extract_sentiment(meeting_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    meeting = meeting_service.get_meeting(db, meeting_id, current_user.id)
